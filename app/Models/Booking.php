...
class Booking extends Model
{
    ...
    protected static function boot()
    {
        ...
        static::created(function ($booking) {
            if (in_array($booking->status, ['paid', 'confirmed', 'pending'])) {
                self::createChecklistForBooking($booking);
            }
        });
    }
    ...
    protected static function createChecklistForBooking(Booking $booking)
    {
        try {
            $template = \App\Models\ChecklistTemplate::where('type', 'property')
                ->where('is_active', true)
                ->first();
            
            if ($template) {
                $checklist = \App\Models\ChecklistResponse::create([
                    'checklistable_type' => get_class($booking),
                    'checklistable_id' => $booking->id,
            ...
            \Log::error('Failed to create checklist for booking: ' . $e->getMessage());
        }
