...
const CarBookingsIndex = () => {
  const { auth, pagination, flash, params } = usePage().props;
  ...
  const handleSearchChange = (e) => {
    ...
    router.get(route('car-bookings.index'), { 
  ...
  const generatePDF = async () => {
    ...
    try {
      const response = await axios.get(route('car-bookings.exportData'), {
      ...
      if (allData.length === 0) {
        toast.error("No car bookings found matching the selected criteria for the PDF export.", { duration: 4000, position: 'top-center' });
        setPdfLoading(false);
        return;
      }
      ...
      logoImg.onload = () => {
        ...
        const columns = ["#", "Booking Number", "Guest Name", "Car Model", "License Plate", "Start Date", "End Date", "Days", "Total Price", "Pickup Location", "Dropoff Location", "Status"];
        const rows = allData.map((data, index) => {
          ...
          return [
            ...
            format(new Date(data.end_date), 'MMM dd, yyyy'),
            days,
            totalPrice,
            data.pickup_location || 'N/A',
            data.dropoff_location || 'N/A',
            data.status || 'N/A'
          ];
        });
        ...
        doc.save(`car_bookings_report_${datePart}.pdf`);
      };
  ...
  const generateExcel = async () => {
    ...
    try {
        ...
        if (allData.length === 0) {
            toast.error("No car bookings found matching the selected criteria for the Excel export.", { duration: 4000, position: 'top-center' });
            return;
        }
        ...
        const ws = XLSX.utils.json_to_sheet(allData.map((data, index) => {
          ...
          return {
            ...
            "Car Model": data.car_name || 'N/A',
            "License Plate": data.license_plate || 'N/A',
            "Start Date": format(new Date(data.start_date), 'MMM dd, yyyy'),
            "End Date": format(new Date(data.end_date), 'MMM dd, yyyy'),
            "Days": days,
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              <input
                ...
                placeholder="Search car bookings..."
              />
              ...
              {pagination.data?.length > 0 ? (
                pagination.data.map((booking) => {
                  ...
                  <tr key={booking.id}>
                    ...
                    <td className="px-6 py-4 whitespace-wrap">KES {parseFloat(booking.car?.platform_price || 0) * days}</td>
                ...
                <tr>
                  <td colSpan="11" className="text-center py-4">No car bookings found.</td>
                </tr>
              )}
            </tbody>
          </table>
