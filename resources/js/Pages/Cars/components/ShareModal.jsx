// @/Components/ShareModal.jsx (Updated for cars)
...
export default function ShareModal({ property, visible, onHide }) {
  const [copied, setCopied] = useState(false);

  // Determine if it's a car or property based on available properties
  const isCar = property?.license_plate || property?.brand || property?.model;
  
  const propertyData = {
    name: property?.property_name || property?.name || "Item",
    type: property?.type || "",
    location: property?.location || "",
    price: property?.platform_price ? `KSh ${property.platform_price}/night` : 
           property?.price_per_day ? `KSh ${property.price_per_day}/day` : "",
    // Use different URLs for cars vs properties
    url: isCar 
      ? `${window.location.origin}/rent-now?car_id=${property.id}`
      : `${window.location.origin}/property-detail?id=${property.id}`,
    // Additional car-specific data
    ...(isCar && {
      brand: property?.brand || "",
      model: property?.model || "",
      year: property?.year || "",
      licensePlate: property?.license_plate || ""
    })
  };

  const getShareText = () => {
    if (isCar) {
      return `Check out "${propertyData.name}" on Ristay - ${propertyData.brand} ${propertyData.model} (${propertyData.year}) for ${propertyData.price}`;
    } else {
      return `Check out "${propertyData.name}" on Ristay - ${propertyData.type} in ${propertyData.location} for ${propertyData.price}`;
    }
  };
  ...
  const shareToFacebook = () => {
    const url = encodeURIComponent(propertyData.url);
    window.open(`https://www.facebook.com/sharer/sharer.php?u=${url}`, '_blank');
  };

  const shareToInstagram = () => {
    navigator.clipboard.writeText(propertyData.url).then(() => {
      alert('Link copied! Paste this link in your Instagram bio, story, or post.');
    });
  };
  ...
  const shareToEmail = () => {
    const subject = encodeURIComponent(`Check out: ${propertyData.name}`);
    const body = encodeURIComponent(`${getShareText()}\n\nView details and book: ${propertyData.url}`);
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl w-full max-w-md mx-auto max-h-[90vh] overflow-y-auto shadow-2xl"
