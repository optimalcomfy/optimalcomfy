...
const EditFood = ({ food, errors }) => {
  ...
  const [preview, setPreview] = useState(food.image ? `/storage/${food.image}` : null);
  ...
  const handleSubmit = (e) => {
    ...
    router.post(route('foods.update', { food: food.id }), formData, {
      forceFormData: true,
      preserveScroll: true,
    });
  };
  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
        <h1 className="text-3xl font-semibold mb-4">Edit Food</h1>
        <form onSubmit={handleSubmit} className="space-y-6" encType="multipart/form-data">
          ...
          <div>
            ...
            {preview && (
              <img src={preview} alt="Preview" className="w-32 h-32 object-cover rounded-md mb-2" />
            )}
            <input
              type="file"
              accept="image/*"
