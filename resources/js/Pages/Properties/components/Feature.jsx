...
const Feature = ({ property }) => {
  const [features, setFeatures] = useState(property?.property_features || []);
  ...
  const handleUploadFeature = async () => {
    ...
    try {
      const response = editingFeature
        ? await axios.post(`/propertyFeatures/${editingFeature.id}`, formData, {
            headers: { "Content-Type": "multipart/form-data" },
          })
        : await axios.post("/propertyFeatures", formData, {
            headers: { "Content-Type": "multipart/form-data" },
          });
  ...
  const handleRemoveFeature = async (featureId) => {
    try {
      await axios.delete(`/propertyFeatures/${featureId}`);
