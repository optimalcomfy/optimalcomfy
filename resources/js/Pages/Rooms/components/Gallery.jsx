...
const Gallery = ({ room }) => {
  const [galleryImages, setGalleryImages] = useState(room?.initial_gallery);
  ...
  const handleUploadImage = async () => {
    ...
    try {
      const response = await axios.post(route("rooms.gallery.store"), formData, {
  ...
  const handleRemoveImage = async (galleryId) => {
    try {
      await axios.delete(route("rooms.gallery.destroy", galleryId));
        ...
        <div className="gallery-grid">
          {galleryImages.map((galleryItem) => (
            <div key={galleryItem.id} className="gallery-item">
              <img src={`/storage/${galleryItem.image}`} alt="Room" className="gallery-img" />
