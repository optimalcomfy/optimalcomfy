...
const Service = ({ room }) => {
  const [services, setServices] = useState(room?.room_services || []);
  ...
  const handleUploadService = async () => {
    ...
    try {
      const response = editingService
        ? await axios.post(
            `/roomServices/${editingService.id}`,  
            formData,
            {
              headers: {
        ...
        : await axios.post("/roomServices", formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          });
  ...
  const handleRemoveService = async (serviceId) => {
    try {
      await axios.delete(route("rooms.services.destroy", serviceId));
